"""
build_fullband_df_figures.py — effort-accounting thread
...
Reads only frozen inputs (archive_v28/expected). Writes to figures/.
The frozen originals are untouched.

Run from the repo root:
    ./venv/Scripts/python.exe effort-accounting/code/build_fullband_df_figures.py
"""
...
ax.set_title("Labor-origin financing of consumption")
...
ax.set_title("Financing origin and production content are different labor linkages")
ax.grid(True, alpha=0.25)
ax.legend(frameon=False, fontsize=8.2, loc="upper right")
fig.tight_layout()
fig.savefig(OUT / "FIG_DFQ_fullband_financing_vs_production_1950_2025.png",
            dpi=250, bbox_inches="tight")
plt.close(fig)
