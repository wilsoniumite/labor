...
of the machine substitute itself.}
\label{fig:schedule}
\end{figure}
...
exists and to participation and scarcity when it does not. A full treatment
would require type-specific task schedules, training costs, and labor
supplies; the historical discussion below uses the mechanism as an
interpretation rather than as an additional result of the baseline model.
...
The model can be used to organize the long record as four schematic
configurations of the relative-productivity schedule
(Figure~\ref{fig:eras}). Because schedules may differ across worker types,
the figure shows each configuration for both an untrained entrant and a
worker with accumulated training or experience. The comparison is
interpretive: the baseline model does not separately solve for the two wage
schedules or for investment in training.
...
\textbf{Pre-industrial: the floor binds.} With machines scarcely present,
nearly every task was closed to them. The machine comparison therefore did
not generally price the wage at an interior task margin, and the binding
boundary of the wage interval was the lower one, tied to the value of exit
and access to land. Demographic adjustment supplied the long-run
equilibration --- the world Ricardo and Malthus described, and the one the
new long-run estimates characterize: pre-industrial English real wages were
strongly shaped by population and land, with productivity growth beginning
well before wages escaped Malthusian pressure (Bouscasse, Nakamura, and
Steinsson 2025). Scarcity and training could support premia for skilled
workers, but those premia are not separately determined in the baseline
model.

\textbf{Industrial: comparative advantage widens.} The Industrial
Revolution created a large and uneven productivity gap across tasks: engines
were far better than muscle at many physical tasks but remained ineffective
at many cognitive and skilled tasks. For less-trained workers, the relevant
margin increasingly lay among tasks that engines could perform, and its
steepness allowed changes in assignment to support substantial wage
differences. Many trained workers still supplied tasks outside the reach of
the available machines, so scarcity and institutions could continue to
support a skill premium. By Proposition~\ref{prop:margin}(ii), a steep
schedule makes employment less responsive to a given wage change and the
wage more responsive to shifts in labor supply. Machine production itself
also remained labor-intensive: in the recursion, both $\gamma(x^*)$ and
$\lambda$ supported the wage. The escape was neither immediate nor smooth
--- real wages grew slowly in the early decades as capital deepening and
urbanisation preceded substantial wage growth (Crafts 2022).

\textbf{Post-industrial: a possible flattening.} Computerization flattened
the routine cognitive region of the schedule --- the task evidence of Autor,
Levy, and Murnane (2003) and the polarization documented by Autor and Dorn
(2013) are the cross-sectional trace. The effect was strongest for workers
concentrated in routine clerical, production, and sales tasks. Many tasks
requiring extensive training remained outside computers' reach, allowing
skill premia to persist even as higher education expanded. AI is a candidate
generalization of the same flattening, and it may also move the margin
computerization barely touched: $\lambda$, the labor used within the machine
sector itself. If AI reaches tasks performed by trained workers while the
relative supply of those workers ceases to bind, the training premium would
tend to narrow. Section~\ref{sec:ai} states the stronger conditions required
for that outcome and what would count against it.

\section{Measurement}\label{sec:measurement}

In this section we observe three empirical objects: the real-wage fork, the size of the rent base relative to a subsistence floor, and the separation between the origin of consumption finance and the human effort embodied in production.
...
\caption{Coverage ratio $\kappa=rT/(N\cdot P_s)$, U.S., 1953--2025. The
...
source-supported windows: annual BEA income-source profiles for financing in
2004--2023 and the BEA full-chain production benchmark in 1997--2023. Values
outside those windows are weaker extensions. In the common source-supported
...
\caption{Composition of U.S. consumption financing outside the owner loop,
1962--2025. The excluded owner loop is consumption financed directly from
household capital income, in which owners consume their own returns. Lines
