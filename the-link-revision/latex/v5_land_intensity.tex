...
% v5_land_intensity — edited 2026-09-01 from the co-authored baseline
% v4_accounting_revised (frozen copy alongside this file). Two changes:
% (1) every zero-non-produced-content ("scarce-factor-free") good
% reframed around land intensity, ideas the sole exact exception, with
% Proposition fork(iv) and the composite pricing display added
% (checks/check_fan.py, ALL GREEN); (2) the financing-versus-production
% detail moved to a new Appendix E (the figure stays in Section 9, her
% call). Round two, same day, her calls: zero-content rhetoric retired
% to Section 2.6 plus one contained close in Section 6; part (i)
% restated on the numeraire; the composites' direct-land definition
% tightened (machine-embodied land rides in the hours term). Word diff
% for review: docs/diff_v4_to_v5_land_intensity.html.
% ============================================================
...
\begin{abstract}
\noindent In a task model the wage is priced against its machine substitute: at the assignment margin, $w=c\cdot\gamma(x^*)$. We endogenize the machine rental through its unit-cost recursion, $c=ac+\lambda w+br$, so margin and recursion jointly give $w=\gamma(x^*)br/(1-a-\lambda\gamma(x^*))$. As machine production sheds labor, rents on non-produced inputs --- permanently land, and over shorter horizons sites, energy, minerals, or bottlenecks --- account for more of replacement cost. We price market exit from the same scarce inputs: unwaged life still requires housing, space, and energy, so rising rents lower the outside option in goods units. When relative machine capability becomes uniform across tasks, the real wage forks by land intensity: nearly pinned by absolute human productivity against the least land-intensive categories, falling without bound against the most, each category leaving the wage's side in order of its intensity as machines improve. In U.S. data since 1964, the two deflated wage measures differ by a factor of 4.8: the same average paycheck buys nearly four times as many durables but about one-fifth less shelter. The model's fiscal completion is a tax on pure rents of non-produced inputs funding a uniform per-person transfer. In the limit it changes neither production nor the work--exit margin. At full capture, measured U.S. site rents would fund about one-third of a per-person subsistence floor in 2025, up from roughly one-twentieth in the 1950s.
\end{abstract}
...
The extreme case makes the mechanism transparent. Suppose relative machine capability becomes uniform across tasks, so no task confers a large comparative advantage on labor. Assignment heterogeneity then disappears: at the market wage, labor and machines have the same unit cost at every task. What happens to purchasing power then depends on where a category of consumption sits on one scale: its land intensity. Deflated by low-intensity categories --- goods that are mostly hours --- the real wage barely moves, held up by absolute human productivity. Deflated by high-intensity categories --- the ground under housing, the acreage under food, the sites under energy --- it falls without bound as machines improve, because that is where the value released by automation settles. Categories leave the wage's side in order of their intensity. (The boundary case --- a good with no claim on scarce inputs at any remove --- is Caselli and Manning's (2019) theorem, exact for a class whose only member in practice is ideas; Section~\ref{sec:limit} concedes it there.) The distinction between a flat capability schedule and the stricter machine-dominance limit is formalized in Section~\ref{sec:limit}; Section~\ref{sec:measurement} shows the corresponding divergence in U.S. price deflators.
...
\end{equation}
falling one-for-one with $q\cdot h_e$ until the dependency floor binds; thereafter, further rent increases do not reduce $s$. The functional form is deliberately minimal; the only essential property is $\partial s/\partial q \le 0$. An exit provisioned partly with goods adds a falling goods term, but the exit bundle sits high in the land-intensity order Section~\ref{sec:limit} builds: an exited person supplies their own hours, and what they cannot supply is the ground under the arrangement --- housing, space, energy, the acreage under food.
...
The same arithmetic prices any consumption category, not only the all-task numeraire. Describe a category by two constants: its \emph{hours content} --- the hours one person would need to produce a unit of it alone, tracing every produced input back through that input's own tasks, a technological constant like $\bar{L}$ --- and its \emph{non-produced content}, the land services its own production draws directly: the acreage under the crop, the site under the dwelling. Land embodied in the machines a category uses is not in the second number: it rides in the machine rental, which parity ties to the wage, and cancels from every ratio below. Its \emph{land intensity} is the second constant relative to the first, and intensity orders categories from ideas, with approximately no content of either kind, to pure location. Take one composite from each end of the order: a low-intensity composite with hours content $\bar{L}_{\lo}$ and non-produced content $b_{\lo}$ per unit (durables are the measured counterpart), and a high-intensity composite with $\bar{L}_{\hi}$ and $b_{\hi}$ (shelter). At cost parity, produced content costs the same whichever input performs it, so each composite's unit cost is
\begin{equation}\label{eq:composites}
p_{\lo} = w\bar{L}_{\lo} + r\, b_{\lo},
\qquad
p_{\hi} = w\bar{L}_{\hi} + r\, b_{\hi}.
\end{equation}
Measured in hours of work, a category's price is its hours plus its land bill converted at $r/w$: the hours of work that buy a unit of land services.
...
\end{equation}
and both automation margins raise $r/w$, without bound as $\bar{\gamma}\to0$. At zero non-produced content the ratio is $1/\bar{L}$ at every level of automation --- part (i)'s case, met exactly only by ideas. At positive content the ratio falls as $r/w$ rises, without bound in the machine-dominance limit, and the non-produced term overtakes the hours term once $r/w$ exceeds the composite's ratio $\bar{L}/b$ --- later, the lower its land intensity $b/\bar{L}$.
\end{proposition}
...
Part (iv) is the fork's working form. A category tracks $1/\bar{L}$ while $r/w$ is small against its hours-to-land ratio and leaves the wage's side after: durables, at the low-intensity end, still track the wage; shelter left it long ago; food sits between. That ordering is the fork's cross-sectional signature, and Section~\ref{sec:measurement} finds it in the deflators (Figure~\ref{fig:fork}). Part (i) is the boundary case, and it contains Caselli and Manning (2019): technological improvement cannot reduce the real wage measured in goods whose inputs are all reproducible. The model concedes the theorem and prices its premise --- no physical category meets it; ideas do --- so the invariance holds as an approximation whose reach part (iv) states, category by category. The fork's welfare effect depends on a measurable elasticity: if households could substitute away from non-produced services, the fork would have little welfare significance; the rising shelter share of low-income budgets indicates which case is empirically relevant (Appendix~\ref{app:landonly} states the CES version and relates the elasticity to shelter shares).
...
The model leaves three empirical objects: the real-wage fork, the size of the rent base relative to a subsistence floor, and the separation between where consumption's purchasing power originates and how much of consumed production resolves into human effort. Appendix~\ref{app:effort} constructs the two accounts behind the third.

\textbf{The fork.} The same average U.S. paycheck, indexed at 1964, buys nearly four times as many durables but about one-fifth less shelter (Figure~\ref{fig:fork}). The ratio between the two deflated wage measures reaches $4.8\times$. This is the observable counterpart of Proposition~\ref{prop:fork}: purchasing power holds in the categories of least land intensity and falls where intensity is highest. Part (iv)'s ordering is also visible between the endpoints: deflated by the food CPI, the same paycheck is up 13 percent over 1964--2024, against 277 percent for durables and $-21$ percent for shelter --- the middle position of a category whose acreage is diluted by processing labor and rising yields. (An energy leg is dominated by world oil prices and carries the shocks rather than the trend; we do not use it.)
