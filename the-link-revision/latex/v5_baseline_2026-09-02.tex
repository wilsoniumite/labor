...
\section{History as three configurations of the model}\label{sec:history}

The model can be applied to the long record as three configurations of the relative-productivity schedule (Figure~\ref{fig:eras}).
...
\caption{Coverage ratio $\kappa = rT/(N\cdot P_s)$ --- site rents $rT$ against population $N$ times the subsistence bundle's cost $P_s$ --- U.S., 1953--2025: median and min--max band across the land-source $\times$ capitalization-rate $\times$ bundle grid.
Dashed: household real estate at market value (Z.1, FRED HNOREMV) minus
residential structures at replacement cost (Z.1, BOGZ1LM155012665Q),
converted to an annual flow with the 10-year Treasury yield (GS10; a +150 bp
variant), in household and economy-wide scopes. Red: PCE housing services
(BEA, DHSGRC1A027NBEA) times a land share of 0.30 or 0.50. All members share
the population times the Orshansky 1963 bundle as a denominator, the Z.1-residual members measure land as market value minus estimated structure replacement cost.}
\label{fig:kappa}
\end{figure}
