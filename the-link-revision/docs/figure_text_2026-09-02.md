# Every word in every figure — latex/v6_walls.tex (2026-09-02)
...
The task-assignment schedule. Tasks are ordered so $\gamma(x)$ increases. On the machine-contestable set, the ratio $w/c$ divides tasks: machines hold tasks with $\gamma(x)<w/c$, labor holds tasks with $\gamma(x)>w/c$, and an interior margin $x^*$ satisfies equality. Tasks in $H$ are closed to machines and appear as a wall at the right edge. If labor holds no machine-contestable task, the machine comparison does not pin the wage (Section~\ref{sec:interval}). A cheaper machine or a lower $\gamma$ near an interior margin shifts tasks toward machines. Task automation changes relative capability at that margin; recursive automation changes the price of the machine substitute itself. The task-assignment margin, or task margin, is the machine--labor indifference threshold; the wall is visual shorthand for tasks excluded from machine assignment, not an additional price schedule.
...
Two labor linkages in U.S. consumption, 1950--2025. The upper series is the share of consumption financed from labor-origin resources; the lower is the share of consumed production that resolves into human effort through the full production chain. Markers identify the strongest benchmark-data windows: annual BEA income-source profiles for financing in 2004--2023 and the BEA full-chain production benchmark in 1997--2023. Values outside those windows are model-based extensions. In the common benchmark-data window, consumption is substantially more labor-financed than production is labor-intensive. The labor-origin financing share tracks where consumption purchasing power originates; the full-chain human-effort share tracks the labor content of consumed production after intermediate inputs are traced recursively. Shaded intervals outside the benchmark-data windows are model-based extensions.
