...
def main():
    ...
    for name, (bridges, truth) in bs.SCEN.items():
        ...
        add(name, old["t"], pol, z, new if truth == "new" else old)
    ...
    add("a pure tightening", t, pol_t, np.column_stack([zz[f"{T:g}Y"] for T in TENORS]), old)
    ...
    pol, z = curves(old, new, rc.learn(old, new, lr))
    add("deficits dominate, slow digestion then AI adoption", old["t"], pol, z, new)
