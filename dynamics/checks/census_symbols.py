# census_symbols.py — defined-symbol census over paper/pinning.html for the
# SYMBOLS EARN THEIR INK rule (brief, 2026-08-19): every defined symbol's
# occurrence count, so single-use definitions are found mechanically, not by
# hand inventory (the ADDENDUM-7 lesson). Informational, not pass/fail:
# a low count is a flag to inline/rework, not automatically a defect —
# formula-bearing one-sentence symbols (v*f/(1-v), eps_D/(eps_D+eps_S)) stay.
# Run: ../venv/Scripts/python.exe checks/census_symbols.py  (from dynamics/)

import re, os

HERE = os.path.dirname(os.path.abspath(__file__))
PAPER = os.path.join(HERE, "..", "paper", "pinning.html")
html = open(PAPER, encoding="utf-8").read()
body = html[html.index("<body>"):html.index('<h2 style="page-break-before: auto;">References')]
# normalize away the italic-variable markup (2026-08-19) so patterns match
body = body.replace("<i>", "").replace("</i>", "")

def lineno(pos):
    return html[:html.index("<body>") + pos].count("\n") + 1

# v2 notation as of 2026-08-27 (docs/notation_map.md): the schedule is gamma,
# interest is rho, wear is delta, the dividend is d, the land coefficient is b,
# the human-essential set is H with measure |H|, shares are alpha/phi, CES
# elasticities sigma and sigma_H, taxes tau_R/tau_w, the floor is s-underbar.
SYMBOLS = {
    "v (verification power, App D)":    [r"verification power v", r"v&middot;f", r"every v &lt; 1"],
    "f (fraud penalty, App D)":         [r"fraud penalty f", r"\bf &rarr; 0", r"v&middot;f"],
    "psi (broadcast fraction, App D)":  [r"&psi;"],
    "|H| (measure of H)":               [r"\|H\|"],
    "w_H (H-wage)":                     [r"w<sub>H</sub>"],
    "sigma (CES goods vs land, A/B)":   [r"&sigma;(?!<sub>H)"],
    "sigma_H (CES H-content, App D)":   [r"&sigma;<sub>H</sub>"],
    "Delta (conditionality, C.1)":      [r"&Delta;"],
    "eps_D/eps_S (elasticities, C.2)":  [r"&epsilon;<sub>D</sub>", r"&epsilon;<sub>S</sub>"],
    "tau_w (payroll tax rate, S8)":     [r"&tau;<sub>w</sub>"],
    "tau_R (rent tax rate, S8)":        [r"&tau;<sub>R</sub>"],
    "d (the dividend, S8)":             [r"transfer d =", r"d &mdash; the", r"\(1&minus;&tau;<sub>R</sub>\)", r"d appears", r"\+ d\b"],
    "omega_ij (ownership shares, S8)":  [r"&omega;"],
    "P (geometric index, Prop 4iii)":   [r"index P =", r"w/P\b"],
    "rho (interest rate, App A)":       [r"&rho;"],
    "delta (wear rate, App A)":         [r"rate &delta;", r"&rho;\+&delta;", r"&rho; \+ &delta;"],
    "X (gross machine services, B.1)":  [r"X =", r"aX\b", r"bX\b"],
    "z (parcel index, B)":              [r"r\(z\)"],
    "N_a (participant count, App A)":   [r"N<sub>a</sub>"],
    "q_enc (C.4)":                      [r"q<sub>enc</sub>"],
    "q* (coverage threshold, C.3)":     [r"q\*"],
    "g_s (bundle goods, C.3)":          [r"g<sub>s</sub>"],
    "h_s (bundle land, C.3)":           [r"h<sub>s</sub>"],
    "n_s (bundle H-service, C.3xD)":    [r"n<sub>s</sub>"],
    "P_s (bundle cost, C.3)":           [r"P<sub>s</sub>"],
    "kappa (coverage)":                 [r"&kappa;"],
    "phi_C (C.5)":                      [r"&phi;<sub>C</sub>"],
    "phi_G (C.5)":                      [r"&phi;<sub>G</sub>"],
    "s_0 (keep, S5)":                   [r"s<sub>0</sub>"],
    "s-underbar (dependency floor, S5)": [r"s&#818;"],
    "h_e (exit land, S5)":              [r"h<sub>e</sub>"],
    "s(q) (exit value, S5)":            [r"s\(q\)"],
    "L_bar (S7)":                       [r"L&#772;"],
    "gamma_bar (S7)":                   [r"&gamma;&#772;"],
    "gamma(x*) (S3)":                   [r"&gamma;</i>\(<i>x</i>\*\)", r"&gamma;\(x\*\)"],
    "x* (S3)":                          [r"x\*"],
    "gamma_L (S3)":                     [r"&gamma;<sub>L</sub>"],
    "gamma_M (S3)":                     [r"&gamma;<sub>M</sub>"],
    "b (land coefficient, S4)":         [r"\bb\b(?=&middot;|<)", r"\bb\b units", r"\bbr\b", r"\bbX\b"],
    "alpha (land weight / share)":      [r"&alpha;"],
    "mu (wedge, S11 note)":             [r"&micro;"],
    "T_H / T_P (App B)":                [r"T<sub>H</sub>", r"T<sub>P</sub>"],
    "T_j / r_j (S8)":                   [r"T<sub>j</sub>", r"r<sub>j</sub>"],
    "H (human-essential set)":          [r"(?<![A-Za-z])H(?![A-Za-z])", r"H-"],
    "m_w/m_e (transfer pair, A/C.1)":   [r"m<sub>w</sub>", r"m<sub>e</sub>"],
    # dead symbols — counts must stay ZERO. First block: killed 2026-08-19
    # (the transfer pair m_w/m_e was killed then REVIVED with a defining home).
    "DEAD m (per-task cost, was App G)":   [r"cost m &rarr;", r"as m &rarr;", r"vanishes with m", r"collapses with m"],
    "DEAD phi_H (was App G)":              [r"&phi;<sub>H</sub>"],
    "DEAD b-prime (was F.1)":              [r"= b &gt; 0", r"&minus;b&prime;"],
    "DEAD y / s(y) (was F.1)":             [r"s\(y\)", r"cash y\b"],
    "DEAD w_f (was App H, cut 2026-08-26)":   [r"w<sub>f</sub>"],
    "DEAD rho_f (was App H, cut 2026-08-26)": [r"&gamma;<sub>f</sub>", r"&rho;<sub>f</sub>"],
    # second block: v1 forms retired by the 2026-08-27 notation pass — any
    # return means a v1 symbol leaked back in (docs/notation_map.md).
    "DEAD ell (v1 land coeff)":         [r"&#8467;"],
    "DEAD eta (v1 CES)":                [r"&eta;"],
    "DEAD beta (v1 broadcast)":         [r"&beta;"],
    "DEAD p_g (v1 goods price)":        [r"p<sub>g</sub>"],
    "DEAD s_d / s_h (v1 forms)":        [r"s<sub>d</sub>", r"s<sub>h</sub>"],
    "DEAD lam_C / lam_R (v1 shares)":   [r"&lambda;<sub>C</sub>", r"&lambda;<sub>R</sub>"],
    "DEAD bare tau (v1 rent tax)":      [r"&tau;(?!<sub>)"],
    "DEAD schedule rho (v1)":           [r"&rho;\*", r"&rho;&#772;", r"&rho;\(x"],
    "DEAD gamma-star (retired 2026-08-28)": [r"&gamma;</i>\*", r"&gamma;\*"],
    "DEAD rho-hat (A-R collision note)": [r"&rho;&#770;", r"&gamma;&#770;"],
    # v1's human-essential K is dead as a SET; the letter returned 2026-08-28
    # as the machine stock (notation map: "frees K for the machine stock"),
    # so only the k-forms stay banned.
    "DEAD k / k_s / w_K (v1 K-set forms)": [r"measure k\b", r"k<sub>s</sub>", r"w<sub>K</sub>"],
    # ---- v2 dynamics symbols (2026-08-28), informational counts
    "u_K / p_K (user cost, build price, S4-5)": [r"u<sub><i>K</i></sub>", r"p<sub><i>K</i></sub>", r"u<sub>K</sub>", r"p<sub>K</sub>"],
    "J (build lag, S4-5)":              [r"<i>J</i>"],
    "theta family (total coefficients)": [r"&theta;"],
    "pi (net rental, S8 only)":         [r"&pi;"],
    "K_t / I_t (stocks and builds, S8)": [r"K<sub><i>t</i></sub>", r"I<sub><i>t</i></sub>"],
    "Q (Hayashi's average Q, S8)":      [r"<i>Q</i>"],
    "h_w (working-life land, S6)":      [r"h<sub><i>w</i></sub>", r"h<sub>w</sub>"],
    "identity matrix (App A)":          [r"&#120793;"],
}

print(f"{'symbol':<38} {'hits':>4}   lines")
print("-" * 72)
rows = []
for name, pats in SYMBOLS.items():
    spans = sorted({mt.start() for p in pats for mt in re.finditer(p, body)})
    lines = sorted({lineno(s) for s in spans})
    rows.append((len(spans), name, lines))
dead_fail = False
for nhits, name, lines in sorted(rows):
    listing = ",".join(map(str, lines[:8])) + ("..." if len(lines) > 8 else "")
    print(f"{name:<38} {nhits:>4}   {listing}")
    if name.startswith("DEAD") and nhits:
        dead_fail = True

print()
if dead_fail:
    print("FAIL: a killed symbol has returned to the body.")
    raise SystemExit(1)
print("dead symbols all zero; counts above are informational")
