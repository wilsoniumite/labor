...
> **2026-09-04 — repository restructured for submission.** This folder was
> `the-link-revision/`. The static paper (her v5 lineage) now lives in `../pinning/`
> with its six figures, `check_pinning.py`, `check_fan.py`, `lint_tex_structure.py`,
> the long draft's corner checks, the Lean formalization, and the effort accounts
> (`../pinning/effort/`, formerly `effort-accounting/`). The sketch blocks, stress
> test and premium-race data moved to `../capability/`; the Swedish fork to
> `../progress_and_prosperity/data_notes/swedish_fork/`; the editing tools to
> `../tools/`. The long draft (`link-repo/`), the v3–v6 TeX baselines, and the
> diff and reading-view HTMLs were deleted; everything is at tag
> `pre-cleanup-2026-09-04`. What remains here is the dynamics thread (logs 31–37);
> every path below this note is as it was when written.
